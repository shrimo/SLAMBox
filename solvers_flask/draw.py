...
class FPS(Node):
    ...
    def out_frame(self):
        ...
        if frame is None:
            print('FPSNode stop')
            return None
        elif self.disabled: return frame
        WH = frame.shape
        fps = str(int(1/(self.new_frame_time-self.prev_frame_time)))
        cv2.rectangle(frame, (int(WH[1]-140), int(WH[0]-50)),
            (int(WH[1]-(70-len(fps)*12)), int(WH[0])), (110,50,30), -1)
        cv2.putText(frame, "fps:" + fps, (int(WH[1]-130), int(WH[0]-20)),
            self.font , 0.75, (self.color_x), 1)
...
class Counter(Node):
    ...
    def out_frame(self):
        ...
        cv2.putText(frame, "frame:" + str(self.frame_counter),
                    (int(30), int(height-20)), self.font , 0.75, self.color, 1)
...
class Text(Node):
    ...
    def out_frame(self):
        ...
        cv2.putText(frame, self.text, (self.px, self.py),
            self.font , self.size, (self.color), 1)
        return frame
...
class Trajectory(Node):
    ...
    def out_frame(self):
        ...
        if frame is None:
            ...
            frame = cv2.polylines(frame, np.int32([self.tr[-self.length:]]), False, self.color, self.size)
        return frame

    def update(self, param):
        self.disabled = param['disabled']
        if self.disabled: self.tr.clear()
