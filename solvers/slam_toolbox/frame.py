...
def featureMappingORB(*frame):
    orb = cv2.ORB_create()
    pts = cv2.goodFeaturesToTrack(np.mean(frame[0], axis=2).astype(np.uint8), 1000, qualityLevel=0.01, minDistance=7, mask=frame[1])
...
class Frame:
    """ Contains poses data """
    def __init__(self, mapp, image, K, pose=np.eye(4), tid=None, verts=None, algorithm='ORB', mask=None):
        self.K = np.array(K)
        self.pose = np.array(pose)
        self.h, self.w = image.shape[0:2]
        self.key_pts, self.descriptors = FT[algorithm](image, mask)
