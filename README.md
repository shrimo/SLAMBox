...
- [MAGSAC++](https://arxiv.org/abs/1912.05909) — an iterative robust estimation method that eliminates the need for a manually defined inlier threshold by marginalizing over the noise scale and weighting each point by its probability of being an inlier, rather than applying a hard boundary.
- [GC-RANSAC](https://arxiv.org/abs/1706.00984) (Graph-Cut RANSAC) — a locally optimized RANSAC variant that uses graph-cut algorithm to refine the inlier set within each iteration, enforcing spatial consistency among neighboring points to produce more accurate and stable model estimates.
